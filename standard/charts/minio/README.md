...
## 默认创建用户
```yaml
hadoop/hadoop_bigdata
```
## 打label指定哪些服务器安装minio
```shell
kubectl label nodes  ks-k8s-master-0  ks-k8s-master-1 ks-k8s-master-2   udp02   udp03 fl002 fl003 fl004 \
 minio-storage=true 
```
## 安装卸载
```shell
...
## 在线扩容
在不改变replicas值的情况下，调大pool参数即可。
然后执行 `helm upgrade minio-01 . -n bigdata-dev    `
